      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
            ...
            // convert to the cars coordinate space
            ...
            auto coeffs = polyfit(x_veh, y_veh, 3); // waypoints fitting
            const double cte = coeffs[0];
            const double epsi = -atan(coeffs[1]);// -f'(0)
            
            //kinematic model used to predict cars state at the moment of control (time + delay dt)
